  ...
  router.post('/api/raw-usage', async (req, res) => {
    ...
    try {
      ...
      if (existingRows.length > 0) {
        ...
        // VALIDATION: Double-check that the selected row is actually for today
        // Convert the existing row's timestamp to Philippine timezone and verify
        const [verifyRows] = await pool.execute(
          "SELECT DATE(CONVERT_TZ(?, @@session.time_zone, '+08:00')) as ph_date",
          [existingTimestamp]
        );
        const existingPhDate = verifyRows.length > 0 ? verifyRows[0].ph_date : null;
        
        // If the selected row is NOT for today, skip UPDATE and go to INSERT
        if (existingPhDate !== todayDateStr) {
          console.log(`⚠️ WARNING: Selected row date (${existingPhDate}) does not match today (${todayDateStr}). This is yesterday's data!`);
          console.log(`   Existing timestamp: ${existingTimestamp}`);
          console.log(`   Skipping UPDATE - will INSERT new row for today instead.`);
          // Clear existingRows to force INSERT path
          existingRows.length = 0;
        } else {
          console.log(`✅ Verified: Selected row is for today (${todayDateStr})`);
        }
        
        // Only proceed with UPDATE if we have a valid row for today
        if (existingRows.length > 0) {
          ...
          // Use date-based WHERE clause to ensure we're updating today's row, not yesterday's
          // This prevents yesterday's data from being updated when it's actually a new day
          // Use UTC_TIMESTAMP() to store in UTC (database standard), but date calculations use Philippine time
          const [updateResult] = await pool.execute(
            "UPDATE rawUsage SET `voltage(V)` = ?, `current(A)` = ?, `power(W)` = ?, `energy(kWh)` = ?, timestamp = UTC_TIMESTAMP() WHERE DATE(CONVERT_TZ(timestamp, @@session.time_zone, '+08:00')) = ? AND timestamp = ?",
            [voltage.toString(), current.toString(), accumulatedPower.toString(), accumulatedEnergy.toString(), todayDateStr, existingTimestamp]
          );
        ...
        } // End of UPDATE block (only executes if row is verified for today)
      }
